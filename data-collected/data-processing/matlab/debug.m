...
%% PARAMETERS
dbDirectory = 'db';
dbFiles = dir(strcat(dbDirectory, '/db3.db'));
dbFilePaths = {}
...
for dbIdx = 1 : numel(dbFiles)
    ...
    packagesNames = {'com.google.android.gms'}
...
for(dbIdx = 1:numel(dbFilePaths))
    ...
    database = databases(dbIdx);
    packagesNames = {'com.google.android.gms'};%allPackageName(database);
    ...
    for(packageIdx = 1:numel(packagesNames))
        ...
        close all;
        if(strcmp(type, 'bar'))
            if(numel([dataX{dbIdx,:,packageIdx}]) > 4 && sum([dataX{dbIdx,:,packageIdx}]) > 0)
                % PLOT BAR
                titleFontSize = 28;
