...
for nameIdx = 1 : numel(dbnames)
    ...
    for packageName = packagesNames
        ...
        idxZero = find(dataX_down_back ~= 0);
        ...
        if(strcmp(type, 'bar'))
            ...
            figureNameUpload = strcat('figures/',dbfilename,'-',packageName, 'scatter-upload.pdf');
            figureNameDownload = strcat('figures/',dbfilename,'-',packageName, 'scatter-download.pdf');
            
            close all;
            fig_upload = plotDataPoint(dataX_back, dataY_back, dataX_fore, dataY_fore, dataX_ibtw, dataY_ibtw, showParam, logYaxis, 'on');

            %title(strcat({'Uploaded data for '},dbfilename,'-',packageName),'FontSize',25);
